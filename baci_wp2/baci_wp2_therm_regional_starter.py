...
cols = 120#240#120#100#1#40#1
rows = 120#240#120#100#1#40#1
N = 1200#1
M = 1200#1
ul_px = 0#250#0#1027#1033#0#1033#0#210
ul_py = 0#100#0#877#893#85#893#0#210
...
for tile in tiles:
    ...
    if 'geog' in os.uname()[1]:
        ...
        root = '/group_workspaces/jasmin2/baci/sigil/%sa1/%s/' % (product_name, tile)
    ...
    for year in xrange(2006, 2016):
        ...
        pattern = product_name.upper() + 'A1_%d.hdf'
        ...
        for ulx in xrange(ul_px, ul_px + N, cols):
            for uly in xrange(ul_py, ul_py + M, rows):
                ...
                if np.logical_or(os.path.isfile(ncd_out) == False, 'geog' in os.uname()[1]):
                    ...
                    com_str = 'python therm_opt_regional.py --ncd_out %s --root %s --root_air %s --pattern %s --year %d --tile %s --ulx %d --uly %d --cols %d --rows %d --step %d' % \
